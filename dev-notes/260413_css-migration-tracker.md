...
| 2026-04-29 | **globals.css vestigial CSS cleanup** — removed `.form-input-base` (migrated one usage in `design-jobs/page.tsx` to `.input-field`); removed dead `prefers-color-scheme` media query duplicates from `.display-field`; replaced `rgba(255,255,255,0.15)` border with `var(--border)`; replaced `#4b5563` disabled text color with `var(--border-secondary)`. |
