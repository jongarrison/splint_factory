...
// GET /api/design-jobs/[id]/mesh-file - Download geometry file for a job
export async function GET(
  ...
  try {
    ...
    if (gpq.meshBlobUrl) {
      ...
      if (gpq.meshBlobUrl.startsWith('/api/local-blob/')) {
        const fs = await import('fs/promises');
        const path = await import('path');
        const storageDir = path.join(process.cwd(), '.blob-storage');
        const filePath = path.join(storageDir, gpq.meshBlobPathname);
        
        const fileBuffer = await fs.readFile(filePath);
        const contentType = getMimeType(gpq.meshFileName as string);
      ...
      return NextResponse.redirect(gpq.meshBlobUrl);
    }
    
    // Fallback to legacy BYTEA storage
    if (!gpq.meshFileContents || !gpq.meshFileName) {
      return NextResponse.json({ error: 'No geometry file available for this job' }, { status: 404 });
    }

    const contentType = getMimeType(gpq.meshFileName as string);
    const filename = safeFilename(gpq.meshFileName as string);
    const buf: Buffer = Buffer.from(gpq.meshFileContents as Buffer);
