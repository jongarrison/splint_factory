...
export async function GET(
  ...
  try {
    ...
    const rows = await prisma.organizationGeometry.findMany({
      ...
      select: { namedGeometryId: true },
    });

    return NextResponse.json(rows.map((r) => r.namedGeometryId));
  } catch (error) {
...
export async function PUT(
  ...
  try {
    ...
    await prisma.$transaction([
      prisma.organizationGeometry.deleteMany({
        where: { organizationId: id },
      }),
      ...(geometryIds.length > 0
        ? [
            prisma.organizationGeometry.createMany({
              data: geometryIds.map((geoId) => ({
                organizationId: id,
                namedGeometryId: geoId,
              })),
            }),
          ]
        : []),
