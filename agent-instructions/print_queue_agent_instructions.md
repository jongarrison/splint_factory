...
* GeometryInputParameterSchema json schema field data (json to be stored in db field described below):
  * See schema defined in: src/designs/design-definition.schema.json#/properties/inputParameters
  * 0 to many member list of:
    * "InputName" - a text field that allows the user to name the geometry parameter. Allowed characters: letters, numbers, underscore
    * "InputDescription" - A text field that captures a description for describing this input parameter
    * "InputType" - a select field that allows choosing "Float", "Integer", "Text", or "Boolean"
...
* Likely fields in NamedGeometry table:
  ...
  * GeometryInputParameterSchema - (as defined in: src/designs/design-definition.schema.json#/properties/inputParameters) a json schema document that describes acceptable input parameter data associated with a print queue entry (described in a separate table).
